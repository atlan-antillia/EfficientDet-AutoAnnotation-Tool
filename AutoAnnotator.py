...
from YOLOAnnotationWriter  import YOLOAnnotationWriter
...
# 2022/05/14 to-arai
flags.DEFINE_string('yolo_output_dir', None, 'Path for yolo image and annotation files.')
flags.DEFINE_string('yolo_classes_file', None, 'Path for classes.txt file.')
flags.DEFINE_bool('yolo_autoannotation', False, 'Enable yolo_autoannotation.')


FLAGS = flags.FLAGS


class AutoAnnotator(ModelInspector):
  ...
  def __init__(self,
    ...
    super().__init__(model_name,
               ...
               hparams,
               **kwargs)
    self.yolo_classes_file = FLAGS.yolo_classes_file
  ...
  def saved_model_inference(self, image_path_pattern, output_dir, **kwargs):
    ...
    images_dir = image_path_pattern 

    pos = image_path_pattern.rfind("/")
    if pos>0:
      images_dir = image_path_pattern[0:pos]
    print("=== saved_model_inference images_dir {}".format(images_dir))
    yolo_output_dir     = FLAGS.yolo_output_dir
    yolo_autoannotation = FLAGS.yolo_autoannotation
    print("=== yolo_output_dir     {}".format(yolo_output_dir))
    print("=== yolo_autoannotation {}".format(yolo_autoannotation))
    
    yolo_master_dir = None
    #2022/05/15 Prepare yolo_output_dir and yolo_master_di
    if yolo_autoannotation:
      if yolo_output_dir !=None:
        ...
        yolo_master_dir     = os.path.join(yolo_output_dir, "master") 
        if os.path.exists(yolo_master_dir):
          shutil.rmtree(yolo_master_dir)
        if not os.path.exists(yolo_master_dir):
          os.makedirs(yolo_master_dir)

      else:
        raise Exception("Not found yolo_output_dir even if yolo_autoannotation is True.")
    ...
    with open(all_predictions_csv, mode="w") as all_prediction_file:
      ...
      for i in range(num_batches):
        ...
        for j in range(size_before_pad):
          ...
          #2022/05/15 Write YOLO an annotation to output_imaget to yolo_master_dir
          if yolo_autoannotation:
            yolo_annotation_writer = YOLOAnnotationWriter(self.yolo_classes_file, images_dir, output_image_path, yolo_master_dir)
            yolo_annotation_writer.write(detected_objects)

          print('=== Writing a image with bboxes and labels to %s' % output_image_path)
    ...
    #2022/05/15 Split master yolo annotated dataset into train and valid
    if yolo_autoannotation:
      splitter = YOLOMasterDatasetSplitter()
      splitted_train_dir = os.path.join(yolo_output_dir, "train")
      splitted_valid_dir = os.path.join(yolo_output_dir, "valid")

      splitter.run(yolo_master_dir, splitted_train_dir, splitted_valid_dir, self.yolo_classes_file)
